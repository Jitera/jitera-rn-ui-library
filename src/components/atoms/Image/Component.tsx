...
import { Image as RnImage, ImageStyle, ImageProps as RnImageProps } from "react-native";
...
export type ImageProps = PropsWithRef<
  Partial<RnImageProps> & {
      ...
      uri?: string;
      source: RnImageProps["source"] | string | React.ElementType<ImageStyle | any>;
      isSVGImage?: boolean;
      style: ImageStyle;
    }
>;
...
const Image = React.forwardRef<RnImage, ImageProps>(
  ({ resizeMode = "cover", source, isSVGImage = false, ImageComponent = RnImage, uri, style, ...props }, ref) => {
    ...
    if (isSVGImage) {
      const Source = source as React.ElementType<ImageStyle | any> ;

      return <Source {...props} width={style?.width} height={style?.height} style={style} />;
    }

    return (
      <ImageComponent {...props} ref={ref} source={newSource} style={style} resizeMode={resizeMode} />
    );
  }
);
